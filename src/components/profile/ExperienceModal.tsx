    ...
    <Modal open={open} onClose={onClose}>
      <form onSubmit={handleSave} className="bg-white rounded-2xl w-full max-w-2xl mx-auto p-4 sm:p-6 md:p-8 flex flex-col">
        <div className="flex items-center justify-between mb-4">
          ...
          {onDelete && (
            <button type="button" className="text-purple-500 font-semibold" onClick={onDelete}>Delete</button>
          )}
        </div>
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-4">
            {EMPLOYMENT_TYPES.map(type => (
              <button
                key={type}
                type="button"
                className={`px-4 py-1 rounded-full border ${form.employmentType === type ? 'bg-gradient-to-r from-[#5B5DE6] to-[#921294] text-white border-transparent' : 'bg-white text-gray-700 border-gray-300'} font-medium transition`}
        ...
        <div className="mb-4">
          ...
          <button type="button" className="text-purple-500 text-sm font-medium mt-1" onClick={handleAddAchievement}>Add More</button>
        </div>
