...
const StorySection = () => {
  return (
    <section className="w-full bg-white flex justify-center relative">
      ...
      <div className="relative w-full max-w-[71.25rem] z-10 mt-[0.5rem]">
        <div className="w-full h-[6.25rem] cursor-pointer sm:h-[9.375rem] md:h-[12.5rem] mt-[2rem] sm:mt-[3rem] md:mt-[3.5rem] flex items-center overflow-x-auto scrollbar-hide px-[0.5rem] space-x-[1rem] sm:space-x-[1.5rem]">
          {profiles.map((profile) => (
            <div
              key={profile.name}
              className="flex flex-col items-center min-w-[4.5rem] sm:min-w-[6.25rem] md:min-w-[8.25rem]"
            >
              <div
              ...
              >
                <div className="w-full h-full bg-white rounded-full flex items-center justify-center overflow-hidden">
                  <img
                    src={profile.img}
                    alt={profile.name}
                    className="w-[4rem] h-[4rem] sm:w-[5.5rem] sm:h-[5.5rem] md:w-[7.75rem] md:h-[7.75rem] object-cover rounded-full"
                  />
                </div>
