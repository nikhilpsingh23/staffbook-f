  ...
  return (
    <section className="w-full bg-white py-24 relative">
      <div className="w-[1093px] mx-auto relative">
        {/* Floating Menu */}
        <div className="absolute top-[-46px] left-1/2 -translate-x-1/2 w-[817px] h-[93px] bg-gradient-to-r from-[#5b5be7] to-[#b14be4] rounded-full shadow-lg flex items-center justify-around px-8 z-10">
        ...
        <div className="mt-[70px] h-[182px] bg-white rounded-[20px] overflow-hidden border border-gray-200 flex flex-col">
          ...
          <div className="grid grid-cols-6 bg-[#F0E5FD] h-[50%]">
            {inputLabels.map((label, index) => (
              <div
                key={index}
                className="flex items-end justify-center px-2 pb-2  text-[13px] font-semibold text-[#a259e6]"
              >
                {label}
              </div>
            ))}
